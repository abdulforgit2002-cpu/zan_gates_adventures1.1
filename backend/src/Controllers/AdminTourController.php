...
class AdminTourController
{
    ...
    private function upsertTourTranslations(
        ...
        $statement = $this->db->prepare(
            "INSERT INTO tour_translations (
                tour_id, language, title, slug,
                short_description, description, duration
            )
            VALUES (
                :tour_id, :language, :title, :slug,
                :short_description, :description, :duration
    ...
    public function updateStatus(int $id): never
    {
        ...
        try {
            ...
            $check = $this->db->prepare("SELECT id FROM tours WHERE id = :id FOR UPDATE");
            $check->execute([':id' => $id]);

            if (!$check->fetchColumn()) {
                $this->db->rollBack();
                Response::error('Tour not found.', 404);
            }
            ...
            error_log('Admin tour status database error: ' . $e->getMessage());
            Response::error('Unable to update tour status at this time.', 500);
        } catch (Throwable $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
            error_log('Admin tour status application error: ' . $e->getMessage());
            Response::error('Unable to update tour status at this time.', 500);
        }
    }
    ...
    public function updateFeatured(int $id): never
    {
        ...
        try {
            ...
            if (!array_key_exists('featured', $body)) {
                Response::error('Featured value is required.', 422);
            }

            $featured = $this->parseBoolean($body['featured']);
            ...
            error_log('Admin tour featured database error: ' . $e->getMessage());
            Response::error('Unable to update featured status at this time.', 500);
        } catch (Throwable $e) {
            error_log('Admin tour featured application error: ' . $e->getMessage());
            Response::error('Unable to update featured status at this time.', 500);
        }
    }
    ...
    public function prices(int $id): never
    {
        ...
        try {
            ...
            $statement = $this->db->prepare("
                ...
                ORDER BY price ASC, min_people ASC, id ASC
            ");
            $statement->execute([':tour_id' => $id]);

            $prices = $statement->fetchAll(PDO::FETCH_ASSOC);
            $this->normalizePrices($prices);

            Response::success(['prices' => $prices], 'Tour prices retrieved successfully.');

        } catch (PDOException $e) {
            error_log('Admin tour prices database error: ' . $e->getMessage());
            Response::error('Unable to retrieve tour prices at this time.', 500);
        } catch (Throwable $e) {
            error_log('Admin tour prices application error: ' . $e->getMessage());
            Response::error('Unable to retrieve tour prices at this time.', 500);
        }
    }
    ...
    public function storePrice(int $id): never
    {
        ...
        try {
            ...
            $pricingType = strtoupper(trim((string) ($body['pricing_type'] ?? '')));
            $minPeople = filter_var($body['min_people'] ?? null, FILTER_VALIDATE_INT);
            $maxPeople = $body['max_people'] ?? null;
            $price = $body['price'] ?? null;
            $currency = strtoupper(trim((string) ($body['currency'] ?? 'USD')));

            $this->validatePriceFields($pricingType, $minPeople, $maxPeople, $price, $currency);

            $statement = $this->db->prepare("
                INSERT INTO tour_prices (
                    tour_id, pricing_type, min_people, max_people, price, currency
                )
                VALUES (
                    :tour_id, :pricing_type, :min_people, :max_people, :price, :currency
                )
                RETURNING id, tour_id, pricing_type, min_people, max_people, price, currency, created_at
            ");

            $statement->bindValue(':tour_id', $id, PDO::PARAM_INT);
            $statement->bindValue(':pricing_type', $pricingType);
            $statement->bindValue(':min_people', $minPeople, PDO::PARAM_INT);
            $statement->bindValue(':max_people', $maxPeople,
                $maxPeople === null ? PDO::PARAM_NULL : PDO::PARAM_INT);
            $statement->bindValue(':price', number_format((float) $price, 2, '.', ''));
            $statement->bindValue(':currency', $currency);
            $statement->execute();

            $createdPrice = $statement->fetch(PDO::FETCH_ASSOC);
            $this->normalizePrice($createdPrice);

            Response::success(['price' => $createdPrice], 'Tour price created successfully.', 201);

        } catch (PDOException $e) {
            error_log('Admin tour store price database error: ' . $e->getMessage());
            Response::error('Unable to create tour price at this time.', 500);
        } catch (Throwable $e) {
            error_log('Admin tour store price application error: ' . $e->getMessage());
            Response::error('Unable to create tour price at this time.', 500);
        }
    }

    public function updatePrice(int $id, int $priceId): never
    {
        AuthMiddleware::requireAdmin();
        $this->validateTourId($id);

        if ($priceId <= 0) Response::error('Invalid price ID.', 422);
        ...
        try {
            ...
            $check = $this->db->prepare("SELECT id FROM tour_prices WHERE id = :id AND tour_id = :tour_id LIMIT 1");
            $check->execute([':id' => $priceId, ':tour_id' => $id]);

            if (!$check->fetchColumn()) Response::error('Tour price not found.', 404);
            ...
            $statement->bindValue(':price', number_format((float) $price, 2, '.', ''));
            $statement->bindValue(':currency', $currency);
            $statement->bindValue(':id', $priceId, PDO::PARAM_INT);
            $statement->bindValue(':tour_id', $id, PDO::PARAM_INT);
            $statement->execute();

            $updatedPrice = $statement->fetch(PDO::FETCH_ASSOC);
            $this->normalizePrice($updatedPrice);

            Response::success(['price' => $updatedPrice], 'Tour price updated successfully.');

        } catch (PDOException $e) {
            error_log('Admin tour update price database error: ' . $e->getMessage());
            Response::error('Unable to update tour price at this time.', 500);
        } catch (Throwable $e) {
            error_log('Admin tour update price application error: ' . $e->getMessage());
            Response::error('Unable to update tour price at this time.', 500);
        }
    }

    public function destroyPrice(int $id, int $priceId): never
    {
        AuthMiddleware::requireAdmin();
        $this->validateTourId($id);
        ...
        try {
            ...
            $statement = $this->db->prepare("
                ...
                WHERE id = :id AND tour_id = :tour_id
            ");
            $statement->execute([':id' => $priceId, ':tour_id' => $id]);

            if ($statement->rowCount() === 0) {
                Response::error('Tour price not found.', 404);
            }

            Response::success(
                ['id' => $priceId, 'tour_id' => $id],
                'Tour price deleted successfully.'
            );

        } catch (PDOException $e) {
            error_log('Admin tour delete price database error: ' . $e->getMessage());
            Response::error('Unable to delete tour price at this time.', 500);
        } catch (Throwable $e) {
            error_log('Admin tour delete price application error: ' . $e->getMessage());
            Response::error('Unable to delete tour price at this time.', 500);
        }
    }
    ...
    public function images(int $id): never
    {
        ...
        try {
            ...
            Response::success(['images' => $images], 'Tour images retrieved successfully.');

        } catch (PDOException $e) {
            error_log('Admin tour images database error: ' . $e->getMessage());
            Response::error('Unable to retrieve tour images at this time.', 500);
        } catch (Throwable $e) {
            error_log('Admin tour images application error: ' . $e->getMessage());
            Response::error('Unable to retrieve tour images at this time.', 500);
        }
    }
    ...
    public function storeImage(int $id): never
    {
        ...
        try {
            ...
            $sortOrder = filter_var($body['sort_order'] ?? 0, FILTER_VALIDATE_INT);
            $sortOrder = $sortOrder !== false && $sortOrder >= 0 ? $sortOrder : 0;
            $isPrimary = $this->parseBoolean($body['is_primary'] ?? false);

            if ($imageUrl === '') Response::error('Image URL is required.', 422);
            if (mb_strlen($imageUrl) > 500) Response::error('Image URL cannot exceed 500 characters.', 422);
            if ($altText !== '' && mb_strlen($altText) > 255) {
                Response::error('Alt text cannot exceed 255 characters.', 422);
            }

            $this->db->beginTransaction();

            if ($isPrimary) {
                $resetPrimary = $this->db->prepare("
                    UPDATE tour_images SET is_primary = FALSE WHERE tour_id = :tour_id
                ");
                $resetPrimary->execute([':tour_id' => $id]);
            }

            $statement = $this->db->prepare("
                INSERT INTO tour_images (tour_id, image_url, alt_text, is_primary, sort_order)
                VALUES (:tour_id, :image_url, :alt_text, :is_primary, :sort_order)
                RETURNING id, tour_id, image_url, alt_text, is_primary, sort_order, created_at
            ");

            $statement->bindValue(':tour_id', $id, PDO::PARAM_INT);
            $statement->bindValue(':image_url', $imageUrl);
            $statement->bindValue(':alt_text', $altText !== '' ? $altText : null,
                $altText !== '' ? PDO::PARAM_STR : PDO::PARAM_NULL);
            $statement->bindValue(':is_primary', $isPrimary, PDO::PARAM_BOOL);
            $statement->bindValue(':sort_order', $sortOrder, PDO::PARAM_INT);
            $statement->execute();

            $createdImage = $statement->fetch(PDO::FETCH_ASSOC);

            $this->db->commit();

            $this->normalizeImage($createdImage);

            Response::success(['image' => $createdImage], 'Tour image created successfully.', 201);

        } catch (PDOException $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
            error_log('Admin tour store image database error: ' . $e->getMessage());
            Response::error('Unable to create tour image at this time.', 500);
        } catch (Throwable $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
            error_log('Admin tour store image application error: ' . $e->getMessage());
            Response::error('Unable to create tour image at this time.', 500);
        }
    }

    public function updateImage(int $id, int $imageId): never
    {
        AuthMiddleware::requireAdmin();
        $this->validateTourId($id);

        if ($imageId <= 0) Response::error('Invalid image ID.', 422);
        ...
        try {
            ...
            $check = $this->db->prepare("
                SELECT id FROM tour_images WHERE id = :id AND tour_id = :tour_id LIMIT 1
            ");
            $check->execute([':id' => $imageId, ':tour_id' => $id]);

            if (!$check->fetchColumn()) Response::error('Tour image not found.', 404);
            ...
            if ($isPrimary) {
                $resetPrimary = $this->db->prepare("
                    ...
                    WHERE tour_id = :tour_id AND id <> :image_id
                ");
                $resetPrimary->execute([':tour_id' => $id, ':image_id' => $imageId]);
            }
            ...
            $updatedImage = $statement->fetch(PDO::FETCH_ASSOC);

            $this->db->commit();

            $this->normalizeImage($updatedImage);

            Response::success(['image' => $updatedImage], 'Tour image updated successfully.');

        } catch (PDOException $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
            error_log('Admin tour update image database error: ' . $e->getMessage());
            Response::error('Unable to update tour image at this time.', 500);
        } catch (Throwable $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
            error_log('Admin tour update image application error: ' . $e->getMessage());
            Response::error('Unable to update tour image at this time.', 500);
        }
    }

    public function destroyImage(int $id, int $imageId): never
    {
        AuthMiddleware::requireAdmin();
        $this->validateTourId($id);
        ...
        try {
            ...
            $statement = $this->db->prepare("
                DELETE FROM tour_images WHERE id = :id AND tour_id = :tour_id
            ");
            $statement->execute([':id' => $imageId, ':tour_id' => $id]);

            if ($statement->rowCount() === 0) {
                Response::error('Tour image not found.', 404);
            }

            Response::success(
                ['id' => $imageId, 'tour_id' => $id],
                'Tour image deleted successfully.'
            );

        } catch (PDOException $e) {
            error_log('Admin tour delete image database error: ' . $e->getMessage());
            Response::error('Unable to delete tour image at this time.', 500);
        } catch (Throwable $e) {
            error_log('Admin tour delete image application error: ' . $e->getMessage());
            Response::error('Unable to delete tour image at this time.', 500);
        }
    }
    ...
    public function destroy(int $id): never
    {
        ...
        try {
            ...
            $check = $this->db->prepare("SELECT id, title FROM tours WHERE id = :id FOR UPDATE");
            $check->execute([':id' => $id]);

            $tour = $check->fetch(PDO::FETCH_ASSOC);
            ...
            $bookingCheck = $this->db->prepare("
                SELECT COUNT(*) FROM booking_enquiries WHERE tour_id = :tour_id
            ");
            $bookingCheck->execute([':tour_id' => $id]);
            ...
            $this->db->prepare("DELETE FROM tour_prices WHERE tour_id = :tour_id")
                ->execute([':tour_id' => $id]);

            $this->db->prepare("DELETE FROM tours WHERE id = :id")
                ->execute([':id' => $id]);

            $this->db->commit();

            Response::success(['id' => $id], 'Tour deleted successfully.');

        } catch (PDOException $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
            error_log('Admin tour delete database error: ' . $e->getMessage());
            Response::error('Unable to delete the tour at this time.', 500);
        } catch (Throwable $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
            error_log('Admin tour delete application error: ' . $e->getMessage());
            Response::error('Unable to delete the tour at this time.', 500);
        }
    }
    ...
    private function validatePriceFields(
        ...
        if ($pricingType === '') Response::error('Pricing type is required.', 422);
        if (mb_strlen($pricingType) > 30) Response::error('Pricing type cannot exceed 30 characters.', 422);

        if (!is_int($minPeople) || $minPeople < 1) {
            Response::error('Minimum people must be at least 1.', 422);
        }

        if ($maxPeople !== null) {
            $maxPeople = filter_var($maxPeople, FILTER_VALIDATE_INT);

            if ($maxPeople === false || $maxPeople < $minPeople) {
                Response::error(
        ...
        if ($price === null || $price === '' || !is_numeric($price)) {
            Response::error('A valid price is required.', 422);
        }

        if ((float) $price <= 0) {
            Response::error('Price must be greater than zero.', 422);
        }

        if (mb_strlen($currency) > 10) Response::error('Currency cannot exceed 10 characters.', 422);
        if ($currency === '') Response::error('Currency is required.', 422);
    }

    /*
    |--------------------------------------------------------------------------
    | BOOLEAN PARSER — RESTORED
    |--------------------------------------------------------------------------
    |
    | Accepts true/false, 1/0, "true"/"false", "yes"/"no", "1"/"0"
    | and returns a strict PHP bool.
    |
    */
    ...
    private function parseBoolean(mixed $value): bool
    {
        ...
        Response::error('Boolean value is invalid.', 422);
    }

    private function normalizePrice(?array &$price): void
    {
    ...
    private function normalizePrices(array &$prices): void
    {
        foreach ($prices as &$price) {
    ...
    private function normalizeImage(?array &$image): void
    {
