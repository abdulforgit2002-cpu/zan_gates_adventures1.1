...
class TourController
{
    ...
    private const SUPPORTED_LANGUAGES = ['en', 'de', 'it', 'fr', 'pl'];
    ...
    {
        $normalized = strtolower(trim((string) ($language ?? 'en')));
    ...
    private function shouldUseTourTranslations(): bool
    {
        try {
            ...
            error_log('Unable to detect tour translation table: ' . $e->getMessage());
            return false;
        }
    }
    ...
    {
        try {
            $language = $this->normalizeLanguage($_GET['lang'] ?? 'en');
            ...
            if ($useTranslations) {
                $stmt = $this->db->prepare(
                    ...
                    LEFT JOIN tour_translations tt
                        ON tt.tour_id = t.id AND tt.language = :language
                    LEFT JOIN LATERAL (
                        SELECT price, currency, pricing_type
                        FROM tour_prices
                        WHERE tour_id = t.id
                        ORDER BY price ASC
                        LIMIT 1
            ...
            foreach ($tours as &$tour) {
                ...
                $tour['featured'] = (bool) $tour['featured'];

                if (isset($tour['price']) && $tour['price'] !== null && $tour['price'] !== '') {
            ...
            Response::success($tours, 'Tours retrieved successfully.');

        } catch (PDOException $e) {
            Response::error('Failed to retrieve tours.', 500, ['error' => $e->getMessage()]);
        }
    }
    ...
    public function showBySlug(string $slug): never
    {
        try {
            ...
            if ($useTranslations) {
                ...
                $stmt->execute([':language' => $language, ':slug' => $slug]);
            } else {
                $stmt = $this->db->prepare(
                    "SELECT
                        t.id, t.title, t.slug, t.short_description, t.description,
            ...
            if ($tour === false) Response::error('Tour not found.', 404);
            ...
            $tour['list_items'] = $tourModel->getListItems((int) $tour['id']);
            $tour['group_prices'] = $tourModel->getGroupPrices((int) $tour['id']);
            $tour['extra_sections'] = $tourModel->getExtraSections((int) $tour['id']);

            Response::success($tour, 'Tour retrieved successfully.');

        } catch (PDOException $e) {
            Response::error('Failed to retrieve tour.', 500, ['error' => $e->getMessage()]);
        }
    }
    ...
    {
        try {
            $tourStmt = $this->db->prepare("SELECT id FROM tours WHERE id = :tour_id AND status = 'ACTIVE'");
            $tourStmt->execute(['tour_id' => $id]);

            if ($tourStmt->fetch() === false) Response::error('Tour not found.', 404);

            $stmt = $this->db->prepare(
                'SELECT id, pricing_type, min_people, max_people, price, currency
                 FROM tour_prices
                 WHERE tour_id = :tour_id
                 ORDER BY price ASC, min_people ASC'
            );
            ...
            foreach ($prices as &$price) {
                ...
                if ($price['max_people'] !== null && $price['max_people'] !== '') {
                    $price['max_people'] = (int) $price['max_people'];
                } else {
            ...
            Response::success($prices, 'Tour prices retrieved successfully.');

        } catch (PDOException $e) {
            Response::error('Failed to retrieve tour prices.', 500, ['error' => $e->getMessage()]);
        }
    }
}