...
/* =========================================================
   CONTACT CHANNELS
   Single source of truth — update here, every page updates.
   ========================================================= */

const CONTACT = {
  whatsappTanzania: "255658450092",
  whatsappUK: "447576096292",
  email: "adventures@zanzibargates.co.tz",
  reservations: "reservations@zanzibargates.co.tz",
  instagram:
    "https://www.instagram.com/zanzibar_gates_safaris._?stkn=MWppNW9rYzU4ZGx3dg%3D%3D&utm_source=qr",
  facebook:
    "https://www.facebook.com/share/19SBfqRvNk/?mibextid=wwXIfr",
  threads:
    "https://www.threads.com/@zanzibar_gates_safaris._?igshid=NTc4MTIwNjQ2YQ==",
};

/* Build a WhatsApp deep link with a prefilled message */
const buildWhatsAppLink = (number, hotelName) => {
...
function HotelDetailPage() {
  ...
  const whatsappTanzaniaLink = buildWhatsAppLink(
    CONTACT.whatsappTanzania,
    hotel.name
  );
  const whatsappUKLink = buildWhatsAppLink(
    CONTACT.whatsappUK,
    hotel.name
  );
          ...
          <div className="hotel-detail-layout">
            ...
            <aside className="hotel-detail-sidebar">
              <div className="hotel-sidebar-card">
                ...
                {/* ============ PRIMARY — WhatsApp Tanzania ============ */}
                <a
                  href={whatsappTanzaniaLink}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hotel-sidebar-button hotel-sidebar-button--whatsapp"
                  aria-label={`Enquire about ${hotel.name} on WhatsApp (Tanzania)`}
                >
                  <svg
                    viewBox="0 0 24 24"
                    width="18"
                  ...
                  {t("hotelDetail.enquireTanzania", "Enquire on WhatsApp (Tanzania)")}
                </a>

                {/* ============ SECONDARY — WhatsApp UK ============ */}
                <a
                  href={whatsappUKLink}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hotel-sidebar-button hotel-sidebar-button--whatsapp-uk"
                  aria-label={`Enquire about ${hotel.name} on WhatsApp (UK)`}
                >
                  <svg
                    viewBox="0 0 24 24"
                    width="18"
                  ...
                  {t("hotelDetail.enquireUK", "Enquire on WhatsApp (UK)")}
                </a>

                {/* ============ EMAIL SHORTCUTS ============ */}
                <div className="hotel-sidebar-emails">
                  <a
                    href={`mailto:${CONTACT.email}?subject=${encodeURIComponent(
                      `Enquiry: ${hotel.name}`
                    )}`}
                    className="hotel-sidebar-email"
                  >
                    ✉️ {CONTACT.email}
                  </a>
                  <a
                    href={`mailto:${CONTACT.reservations}?subject=${encodeURIComponent(
                      `Enquiry: ${hotel.name}`
                    )}`}
                    className="hotel-sidebar-email"
                  >
                    ✉️ {CONTACT.reservations}
                  </a>
                </div>
                ...
                <div className="hotel-sidebar-socials">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hotel-sidebar-social hotel-sidebar-social--threads"
                    aria-label={`Message ${hotel.name} on Threads`}
                    title="Threads"
                  >
                    <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true">
