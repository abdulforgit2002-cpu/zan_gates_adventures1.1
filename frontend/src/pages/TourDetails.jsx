...
const normalizeTour = (response) => {
    ...
    if (data.tour) {
        ...
        return {
            ...
            group_prices:
                data.tour.group_prices ??
                data.group_prices ??
                [],

            extra_sections:
                data.tour.extra_sections ??
                data.extra_sections ??
                {},
        };
    }
...
function TourDetails() {
    ...
    useEffect(() => {
        ...
        const loadTour = async () => {
            ...
            try {
                ...
                if (!slug) {
                    throw new Error(t("validation.noTourSpecified"));
                }

                const response = await getTourBySlug(slug);
                const normalizedTour = normalizeTour(response);

                if (!normalizedTour) {
                    throw new Error(t("validation.tourNotFound"));
                }
                ...
                /* PRICES */
                let loadedPrices = normalizeArray(normalizedTour.prices);

                if (loadedPrices.length === 0 && normalizedTour.id) {
                    const priceResponse = await getTourPrices(normalizedTour.id);
                    loadedPrices = normalizeArray(priceResponse);
                }

                /* IMAGES */
                let loadedImages = normalizeArray(normalizedTour.images);

                if (loadedImages.length === 0 && normalizedTour.id) {
                    const imageResponse = await getTourImages(normalizedTour.id);
                    loadedImages = normalizeArray(imageResponse);
                }
                ...
                const primary = getPrimaryImage(loadedImages);
                setSelectedImage(primary);

                const primaryIndex = loadedImages.findIndex(
                    (image) => getImageUrl(image) === primary
                );

                setImageIndex(primaryIndex >= 0 ? primaryIndex : 0);
                ...
                if (mounted) {
                    setError(err?.message || t("validation.loadFailed"));
                }
        ...
        if (slug) {
            ...
            setLoading(false);
            setError(t("validation.noTourSpecified"));
        }
    ...
    */
    ...
    }, [lightboxOpen, imageIndex, images]);
    ...
    */
    ...
    const startingPrice = useMemo(() => {
        ...
        const validPrices = prices.filter(
            (price) => Number.isFinite(Number(price?.price))
        );
    ...
    */
    ...
    const estimatedPrice = useMemo(() => {
        ...
        if (!prices.length || guestCount <= 0) return null;

        const perPersonPrices = prices
            .filter(isPerPerson)
            .filter((price) => Number.isFinite(Number(price.price)))
            .sort((a, b) =>
                Number(a.min_people || 1) - Number(b.min_people || 1)
            );

        const matching = perPersonPrices.find((price) => {

            const min = Number(price.min_people || 1);
        ...
        if (matching) {
            return {
                amount: Number(matching.price) * guestCount,
                currency: matching.currency || currency,
            };
        }

        if (perPersonPrices.length) {
            const fallback = perPersonPrices[0];
            return {
                amount: Number(fallback.price) * guestCount,
    ...
    */
    ...
    const formatMoney = (amount, moneyCurrency) => {
        ...
        return `${symbol}${Number(amount).toLocaleString(currentLocale, {
    ...
    */
    ...
    const handleSubmit = async (event) => {
        ...
        if (!fullName) { setBookingError(t("validation.fullNameRequired")); return; }
        if (!email) { setBookingError(t("validation.emailRequired")); return; }
        if (!form.travel_date) { setBookingError(t("validation.travelDateRequired")); return; }
        if (form.travel_date < todayString()) { setBookingError(t("validation.travelDatePast")); return; }
        if (!Number.isInteger(adults) || adults < 1) { setBookingError(t("validation.adultRequired")); return; }
        if (!Number.isInteger(children) || children < 0) { setBookingError(t("validation.childrenInvalid")); return; }
        if (!tour?.id) { setBookingError(t("validation.tourUnavailable")); return; }
        ...
        try {
            ...
            const payload = {
                ...
                estimated_total: estimatedPrice ? estimatedPrice.amount : null,
                currency: estimatedPrice?.currency || currency,
            };

            const response = await createBookingEnquiry(payload);

            console.log("Booking enquiry created:", response);
            ...
            setBookingError(err?.message || t("validation.submitFailed"));
        ...
        return (
            <div className="tour-details-error">
                <div className="tour-details-error-inner">
                    ...
                    <p>{error || t("tourDetails.errorDescription")}</p>

                    <Link to="/#tours" className="tour-error-button">
        ...
        <div className="tour-details-page">
            ...
            >
                ...
                <div className="tour-details-container">
                    <div className="tour-details-breadcrumb">
                        ...
                        <span>•</span>
                        <Link to="/tours">{t("navigation.tours")}</Link>
                        <span>•</span>
                    ...
                    <div className="tour-details-layout">
                        ...
                        <div className="tour-details-main">
                            ...
                            <section className="tour-details-section tour-gallery-section">
                                <div className="tour-gallery-heading">
                                    <div>
                                        ...
                                        <h2>{t("tourDetails.gallery.title")}</h2>
                                    </div>
                                ...
                                {primaryImage ? (
                                    <div className="tour-gallery">
                                        <button
                                            ...
                                            aria-label={t("tourDetails.gallery.openImage")}
                                        >
                                            <img
                                                src={primaryImage}
                                                alt={getImageAlt(images[imageIndex], tour.title)}
                                                onError={(event) => {
                                            ...
                                            {images.length > 1 && (
                                                <>
                                                    <button
                                                        ...
                                                        aria-label={t("tourDetails.gallery.previousImage")}
                                                    >
                                                        ←
                                                    </button>
                                                    <button
                                                        ...
                                                        aria-label={t("tourDetails.gallery.nextImage")}
                                                    >
                                                        →
                                                    </button>
                                                </>
                            ...
                            <section className="tour-details-section">
                                ...
                                <h2>{t("tourDetails.aboutTitle")}</h2>
                                ...
                                <div className="tour-description">
                                    {tour.description ? (
                                        ...
                                        <p>{t("tourDetails.defaultDescription")}</p>
                                    )}
                                </div>
                            </section>
                            ...
                            {(tour.duration_start ||
                                ...
                                <section className="tour-details-section">
                                    <span className="tour-section-eyebrow">
                                        {t("tourDetails.practicalInfo", "PRACTICAL INFO")}
                                    </span>
                                    <h2>
                                        {t("tourDetails.practicalInfoTitle", "Timing & logistics")}
                                    </h2>

                                    <div className="tour-info-grid">
                                        {tour.duration_start && tour.duration_end && (
                                            <div className="tour-info-card">
                                                <span className="tour-info-number">01</span>
                                                <div>
                                                    <span>{t("tourDetails.duration")}</span>
                                                    <strong>
                                                        {tour.duration_start} – {tour.duration_end}
                                                    </strong>
                                                </div>
                                        ...
                                        {tour.departure_location && (
                                            <div className="tour-info-card">
                                                <span className="tour-info-number">02</span>
                                                <div>
                                                    <span>
                                                        {t("tourDetails.departureFrom", "Departure from")}
                                                    </span>
                                                    <strong>{tour.departure_location}</strong>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {tour.return_location && (
                                            <div className="tour-info-card">
                                                <span className="tour-info-number">03</span>
                                                <div>
                                                    <span>
                                                        {t("tourDetails.returnTo", "Return to")}
                                                    </span>
                                                    <strong>{tour.return_location}</strong>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                            ...
                            {tour.list_items?.highlight?.length > 0 && (
                                <section className="tour-details-section">
                                    <span className="tour-section-eyebrow">
                                        {t("tourDetails.highlightsEyebrow", "WHY YOU'LL LOVE IT")}
                                    </span>
                                    <h2>
                                        {t("tourDetails.highlightsTitle", "Tour highlights")}
                                    </h2>

                                    <div className="tour-benefits">
                                        {tour.list_items.highlight.map((text, index) => (
                                            <div key={index}>
                            ...
                            {(tour.list_items?.include?.length > 0 ||
                                ...
                                <section className="tour-details-section">
                                    <span className="tour-section-eyebrow">
                                        {t("tourDetails.includesEyebrow", "WHAT'S INCLUDED")}
                                    </span>
                                    <h2>
                                        {t("tourDetails.includesTitle", "Inclusions & exclusions")}
                                    </h2>

                                    <div className="tour-inclusions">
                                        {tour.list_items?.include?.length > 0 && (
                                            <div className="tour-inclusions-column">
                                                <h3>{t("tourDetails.included", "Included")}</h3>
                                                <ul>
                                                    {tour.list_items.include.map((text, i) => (
                                                        <li key={i}>
                                        ...
                                        {tour.list_items?.exclude?.length > 0 && (
                                            <div className="tour-inclusions-column tour-inclusions-column--excludes">
                                                <h3>{t("tourDetails.excluded", "Not included")}</h3>
                                                <ul>
                                                    {tour.list_items.exclude.map((text, i) => (
                                                        <li key={i}>
                                                            <span aria-hidden="true">×</span>
                                                            {text}
                                                        </li>
                            ...
                            {tour.list_items?.activity?.length > 0 && (
                                <section className="tour-details-section">
                                    <span className="tour-section-eyebrow">
                                        {t("tourDetails.activitiesEyebrow", "WHAT YOU'LL DO")}
                                    </span>
                                    <h2>{t("tourDetails.activitiesTitle", "Activities")}</h2>

                                    <div className="tour-info-grid">
                                        {tour.list_items.activity.map((text, i) => (
                                            <div key={i} className="tour-info-card">
                            ...
                            {tour.list_items?.what_to_see?.length > 0 && (
                                <section className="tour-details-section">
                                    <span className="tour-section-eyebrow">
                                        {t("tourDetails.whatToSeeEyebrow", "WHAT YOU'LL SEE")}
                                    </span>
                                    <h2>{t("tourDetails.whatToSeeTitle", "What to see")}</h2>

                                    <div className="tour-info-grid">
                                        {tour.list_items.what_to_see.map((text, i) => (
                                            <div key={i} className="tour-info-card">
                                                <span className="tour-info-number">
                                                    {String(i + 1).padStart(2, "0")}
                                                </span>
                            ...
                            {tour.group_prices?.length > 0 && (
                                <section className="tour-details-section">
                                    <span className="tour-section-eyebrow">
                                        {t("tourDetails.groupPricingEyebrow", "GROUP PRICING")}
                                    </span>
                                    <h2>
                                        {t("tourDetails.groupPricingTitle", "Price details")}
                                    </h2>

                                    <div className="tour-group-pricing">
                                        <div className="tour-group-pricing-header">
                                            <span>{t("tourDetails.noOfPeople", "No of people")}</span>
                                            <span>{t("tourDetails.groupPrice", "Group price")}</span>
                                        </div>

                                        {tour.group_prices.map((row) => (
                                            <div key={row.id} className="tour-group-pricing-row">
                                                <span>{row.label}</span>
                                                <strong>
                            ...
                            {Object.entries(tour.extra_sections || {}).map(([key, section]) => (
                                <section key={key} className="tour-details-section">
                                    ...
                                    <h2>{section.title || key}</h2>

                                    <div className="tour-extra-sections">
                                        {section.items.map((item, i) => (
                                            <div key={i} className="tour-extra-item">
                                                {item.title && (
                                                    <strong className="tour-extra-item-title">
                                                        {item.title}
                                                    </strong>
                                                )}
                                                {item.body && (
                                                    <p className="tour-extra-item-body">
                                                        {item.body}
                                                    </p>
                                                )}
                                            </div>
                            ...
                            <section className="tour-details-section">
                                ...
                                <h2>{t("tourDetails.informationTitle")}</h2>

                                <div className="tour-info-grid">
                                    <div className="tour-info-card">
                                        <span className="tour-info-number">01</span>
                                        <div>
                                            <span>{t("tourDetails.destination")}</span>
                                            <strong>
                                                {tour.destination_name ||
                                                    tour.destination?.name ||
                                                    t("common.zanzibar")}
                                    ...
                                    <div className="tour-info-card">
                                        <span className="tour-info-number">03</span>
                                        <div>
                                            <span>{t("tourDetails.category")}</span>
                                            <strong>
                                                {tour.category_name ||
                                                    tour.category?.name ||
                                                    t("common.zanzibarAdventure")}
                                    ...
                                    <div className="tour-info-card">
                                        <span className="tour-info-number">04</span>
                                        <div>
                                            <span>{t("tourDetails.availability")}</span>
                                            <strong>{t("tourDetails.dailyEnquiry")}</strong>
                                        </div>
                                    </div>
                                </div>
                            </section>
                            ...
                            <section className="tour-details-section">
                                ...
                                <h2>{t("tourDetails.pricingTitle")}</h2>
                                ...
                                {prices.length > 0 ? (
                                    ...
                                    <div className="tour-no-pricing">
                                        <strong>{t("tourDetails.pricingOnRequest")}</strong>
                                        <span>
                            ...
                            <section className="tour-details-section tour-benefits-section">
                                ...
                                <h2>{t("tourDetails.localTeamTitle")}</h2>
                        ...
                        <aside className="tour-details-sidebar">
                            <div className="tour-booking-card">
                                <div className="tour-booking-top">
                                    ...
                                    {startingPrice && (
                                        <div className="tour-booking-starting-price">
                                            <small>{t("tourDetails.booking.from")}</small>
                                            <strong>
                                                {formatMoney(
                                                    startingPrice.price,
                                                    startingPrice.currency
                                ...
                                <h3>{t("tourDetails.booking.enquireTitle")}</h3>
                                <p>{t("tourDetails.booking.enquireDescription")}</p>
                                ...
                                {submitted ? (
                                    <div className="booking-success">
                                        ...
                                        <h4>{t("tourDetails.booking.thankYou")}</h4>
                                        <p>
                                    ...
                                    <form className="tour-booking-form" onSubmit={handleSubmit}>
                                        ...
                                        <div className="booking-field">
                                            ...
                                            <input
                                                ...
                                                placeholder={t("tourDetails.booking.fullNamePlaceholder")}
                                                autoComplete="name"
                                                required
                                            />
                                        </div>
                                        ...
                                        <div className="booking-field">
                                            ...
                                            <input
                                                ...
                                                value={form.email}
                                                onChange={handleChange}
                                                placeholder={t("tourDetails.booking.emailPlaceholder")}
                                                autoComplete="email"
                                                required
                                            />
                                        </div>
                                        ...
                                        <div className="booking-field">
                                            <label htmlFor="phone">
                                                ...
                                                <span>{t("tourDetails.booking.optional")}</span>
                                            </label>
                                            <input
                                                id="phone"
                                                type="tel"
                                                ...
                                                value={form.phone}
                                                onChange={handleChange}
                                                placeholder={t("tourDetails.booking.phonePlaceholder")}
                                                autoComplete="tel"
                                            />
                                        </div>
                                        ...
                                        <div className="booking-field">
                                            ...
                                            <textarea
                                                ...
                                                onChange={handleChange}
                                                rows="4"
                                                placeholder={t("tourDetails.booking.messagePlaceholder")}
                                            />
                                        </div>
            ...
            {lightboxOpen && primaryImage && (
                <div
                    ...
                    role="dialog"
                    aria-modal="true"
                    aria-label={t("tourDetails.gallery.tourImageGallery")}
                ...
                >
                    <button
                        ...
                        onClick={() => setLightboxOpen(false)}
                        aria-label={t("tourDetails.gallery.closeGallery")}
                    >
                        ×
                    </button>
